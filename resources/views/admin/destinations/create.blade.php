<x-admin-master-layout>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="destinationForm" class="row g-3" method="POST" action="{{ isset($data) ? route('admin_destinations.update_update', $data->id) : route('admin_destinations.create_create') }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label for="quill-overview" class="form-label">Overview</label>
                                <div id="quill-overview"></div>
                                <input type="hidden" id="overview" name="overview" value="{{ $data->overview ?? '' }}">
                            </div>
                            <div class="mb-4"></div>
                            <div class="mb-2"></div>
                            <div class="mb-3">
                                <label for="quill-info" class="form-label">Info</label>
                                <div id="quill-info"></div>
                                <input type="hidden" id="info" name="info" value="{{ $data->info ?? '' }}">
                            </div>
                            <div class="mb-4"></div>
                            <div class="mb-2"></div>
                            <div class="mb-3">
                            ...
                            <div>
                                <button type="submit" id="submit-button" class="btn btn-primary">{{ isset($data) ? 'Save' : 'Create' }}</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        // this is a temporary solution. I'll move it soon!
        
        $(document).ready(function() {
            let tourDetailCount = {{ isset($data) ? count(json_decode($data->tour_details)) : 1 }};
            ...
            let imageCount = {{ isset($data) ? count(json_decode($data->images)) : 1 }};
            ...
            var quillOverview = new Quill('#quill-overview', {
                theme: 'snow'
            });
            var quillInfo = new Quill('#quill-info', {
                theme: 'snow'
            });

            quillOverview.root.innerHTML = $('#overview').val();
            quillInfo.root.innerHTML = $('#info').val();

            $('#submit-button').click(function(e) {
                $('#overview').val(quillOverview.root.innerHTML);
                $('#info').val(quillInfo.root.innerHTML);
            });
        });
    </script>
</x-admin-master-layout>