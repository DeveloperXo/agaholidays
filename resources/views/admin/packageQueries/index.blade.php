    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Package Queries</li>
            </ol>
        </nav>
    </div>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $d->phone }}</td>
                                    <td>{{ $d->destination }}</td>
                                    <td><a href="{{ isset($d->package) ? route('user_packages_single', $d->package->id) : '#' }}">{{ isset($d->package) ? substr($d->package->package_name, 0, 10).'...' : '-' }}</a></td>
                                    <td><a href="{{ isset($d->user) ? route('admin_users.update_view', $d->user->id) : '#' }}">{{ isset($d->user) ? substr($d->user->name, 0, 10).'...' : '-' }}</a></td>
                                    <td>{{ isset($d->created_at) ? Carbon::parse($d->created_at)->format('d-m-Y') : '-' }}</td>
                                    <td>
                                        <a href="{{ route('admin_package_queries.update_view', $d->id) }}" class="btn btn-primary btn-sm">View</a>
                                        <x-bs-modal
                                            ...
                                            modal_btn="Delete"
                                        >
                                            <form id="deleteQueryForm{{ $d->id }}" action="{{ route('admin_package_queries.delete', $d->id) }}" method="post">
                                                @csrf
                                                @method('delete')
                                            </form>
                                        </x-bs-modal>
